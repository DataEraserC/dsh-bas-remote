...
export class DevChannelTunnel {
   ...
   * @param {number} [options.connectAttempts] - how often to retry a failed handshake.
  ...
  constructor({
    ...
    this.session = null
    this.forwarder = null
    /** @type {Map<number, object>} extra loopback forwards, keyed by remote port. */
    this.extraForwards = new Map()
    this.startedAt = null
    this.closedReason = ''
    this.closeWaiters = []
  }
   ...
   */
  async start() {
    // The dev channel answers "Failed to read the protocol version" and similar
    // handshake errors intermittently (a fresh dev space and a busy one both
    // do), so one failure is retried with a small backoff before giving up.
    const attempts = Math.max(1, Math.min(5, Number(this.connectAttempts) || 3))
    let lastError
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      try {
        ...
        this.closedReason = ''
        if (attempt < attempts) {
          this.log('debug', `dev channel attempt ${attempt} failed (${error.message}); retrying`)
  ...
  /**
   * Forward one more dev-space loopback port over the same dev channel.
   *
   * Used by the SFTP bridge, whose server listens on a second port inside the
   * dev space. Creating the same remote port twice returns the existing
   * forward.
   * @param {number} remotePort - port inside the dev space.
   * @param {number} [requestedLocalPort] - loopback port; 0 picks a free one.
   * @returns {Promise<number>} the loopback port carrying that remote port.
   */
  async addForward(remotePort, requestedLocalPort = 0) {
    const existing = this.extraForwards.get(remotePort)
    if (existing) return existing.localPort
    if (!this.session) throw new Error('cannot add a forward before the dev channel is up')
    const forwarding = this.session.activateService(PortForwardingService)
    const forwarder = await forwarding.forwardToRemotePort('127.0.0.1', requestedLocalPort, '127.0.0.1', remotePort)
    this.extraForwards.set(remotePort, forwarder)
    this.log('debug', `forwarding 127.0.0.1:${forwarder.localPort} to dev-space port ${remotePort}`)
    return forwarder.localPort
  }

  /**
   * Drop one extra forward.
   * @param {number} remotePort - port inside the dev space.
   * @returns {Promise<void>} resolves once disposal is requested.
   */
  async removeForward(remotePort) {
    const forwarder = this.extraForwards.get(remotePort)
    if (!forwarder) return
    this.extraForwards.delete(remotePort)
    try {
  ...
  async stop() {
    ...
    this.forwarder = null
    for (const forwarder of this.extraForwards.values()) {
      try {
        await forwarder.dispose?.()
      } catch {
        // The session may already be gone.
