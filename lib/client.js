...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function DevSpaceRow(props) {
      ...
      // Lifecycle control (start/stop) plus tunnel state
      var controls = []
      ...
      if (tunnel) {
        controls.push(React.createElement('span', { key: 'tunnel-dot', style: { display: 'inline-block', width: 8, height: 8, borderRadius: '50%', backgroundColor: tunnel.active ? T.ok : T.danger, flexShrink: 0 } }))
        controls.push(React.createElement('span', { key: 'endpoint', style: { color: T.textMuted } }, tr('sshEndpoint') + ': 127.0.0.1:' + tunnel.localPort))
      ...
      } else if (!ds.sshEnabled) {
        controls.push(React.createElement('span', { key: 'hint', style: { color: T.textMuted, fontSize: 11 } }, tr('sshHint')))
      } else if (!running) {
        controls.push(React.createElement('span', { key: 'stopped', style: { color: T.textMuted, fontSize: 11 } }, tr('startFirst')))
      ...
      return React.createElement('div', { key: ds.id }, row, detail)
    }
