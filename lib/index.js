...
export function apply(ctx, config) {
  ...
  async function connectDevSpace({ landscape: landscapeInput, devSpace, localPort }) {
    ...
    if (!picked.sshEnabled) {
      throw new Error(
        `dev space "${picked.label || picked.id}" does not expose SSH; add the "Remote Access" extension to the dev space in BAS and restart it`,
      )
    }
  ...
  const tools = [
    ...
    defineTool({
      name: 'bas_connect',
      description:
        'Connect a BAS dev space and expose it as a local SSH endpoint: fetches the dev-space private key, opens the dev-channel tunnel and publishes a ~/.ssh/config entry. A stopped dev space is started first (as the SAP extension does), then waited for. The result names the loopback host/port, the key file and the `rw_connect` call that adopts the dev space as a remote workspace.',
      parameters: {
        devSpace: { type: 'string', required: true, description: 'Dev space display name or id (see bas_devspaces)' },
        landscape: { type: 'string', description: 'Landscape URL or host (optional when exactly one is configured)' },
  ...
  ctx.commands.register({
    name: 'bas',
    description:
      'SAP Business Application Studio remote dev spaces (status | login | logout | devspaces | start | stop | connect | disconnect)',
    input: {
      hint: 'status | login <landscape> | devspaces [landscape] | start <dev space> | stop <dev space> | connect <dev space> | disconnect [dev space] | logout <landscape>',
    },
